...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (tg) {
      ...
      tg.setHeaderColor('#0A0A0B'); // Красим системный хэдер
      tg.setBackgroundColor('#050506');
      
      // Блокируем свайп вниз для закрытия (критично для скролла в чате)
  ...
  // ============================================
  // 🎨 РЕНДЕР (ЖЕЛЕЗОБЕТОННЫЙ КАРКАС)
  // ============================================
  return (
    // КОРЕНЬ: Зафиксирован на весь экран (никакого наружного скролла)
    <div className="fixed inset-0 w-full bg-[#050506] text-white flex flex-col overflow-hidden select-none">
      ...
      {/* ВЕРХНЯЯ ПЛАШКА (ХЭДЕР): Зафиксирована, не сжимается */}
      ...
      {/* ЦЕНТРАЛЬНАЯ ЗОНА: Единственное место, где разрешен скролл */}
      <div className="flex-1 overflow-y-auto relative custom-chat-scroll flex flex-col">
        ...
        {/* ЭКРАН 2: ЧАТ (Растягивается абсолютно на всю центральную зону) */}
        {activeTab === 'chat' && (
            ...
            <div className="flex-1 p-4 space-y-3 overflow-y-auto bg-[#08080a] custom-chat-scroll">
              {chatMessages.map(msg => (
                <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.sender === 'system' ? (
                    <div className="bg-white/5 text-gray-400 text-xs p-3 rounded-xl text-center max-w-full mx-auto">{msg.text}</div>
                  ) : msg.type === 'media' ? (
      ...
      {/* НИЖНЕЕ МЕНЮ НАВИГАЦИИ: Скрываем внутри чата, чтобы не мешало клавиатуре */}
      {activeTab !== 'chat' && (

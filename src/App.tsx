...
import {
  ...
  Zap, Copy, Info, CheckCircle2, LockKeyhole, AlertTriangle, ChevronRight
} from 'lucide-react';
...
const STRANGER_PERSONAS: StrangerPersona[] = [
  {
    ...
    bio: 'Художница, люблю урбанизм и кофе. Верю в случайные встречи.',
  ...
  },
  {
    ...
    bio: 'Разработчик, люблю путешествия и крафт. Ценю адекватность.',
...
const checkAgeVerification = () => {
  ...
  if (!telegramUser) return { allowed: true };
  const userAge = (telegramUser as any).age || 18;
  if (userAge < 18) return { allowed: false, reason: 'underage' };
  return { allowed: true };
};

const moderateContent = async (content: string, type: 'text' | 'image' = 'text') => {
  const toxicKeywords = ['оскорбление', 'спам', 'мошенник'];
  if (type === 'text') {
    const hasToxic = toxicKeywords.some(k => content.toLowerCase().includes(k));
    if (hasToxic) return { blocked: true, reason: 'toxic_content' };
  }
  return { blocked: false };
};

const calculateTrustScore = (karma: number, reports: number, accountAge: number) => {
  let score = 50;
  score += Math.min(karma * 0.5, 30);
  score += Math.min(accountAge / 10, 10);
  score -= reports * 15;
...
export default function App() {
  ...
  const [filters, setFilters] = useState<Filters>({
    gender: 'all', country: 'all', ageRange: [18, 35]
  });
      ...
      if (currentProgress >= 100) {
        ...
        triggerSmokeEffect('match', () => {
          ...
          const systemMsg: ChatMessage = {
            ...
            text: '⚠️ Чат анонимен. Нажмите 🔥 для экстренного удаления переписки.',
          ...
          setActiveTab('chat');
        }, `Соединение с Незнакомцем ${selectedStranger.tag}...`);
      }
    }, 200);
  };
  ...
  const handleSendMessage = (e?: React.FormEvent, customMedia?: { type: 'media' | 'voice', content: string }) => {
    ...
    setTimeout(() => {
      ...
      let replyText = '';
      if (isMedia) {
        replyText = 'Ого, медиа! Выглядит круто! 😍';
      } else {
        const lowerText = text.toLowerCase();
        const matched = selectedStranger.replies.find(r => 
          r.keywords.some(k => lowerText.includes(k))
        );
        replyText = matched ? matched.text : selectedStranger.defaultReplies[Math.floor(Math.random() * selectedStranger.defaultReplies.length)];
      }

      const strangerMsg: ChatMessage = {
        id: `stranger-msg-${Date.now()}`, sender: 'stranger',
        text: replyText,
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
  ...
  const handleExtendLimit = () => {
    handleSpendStars(15, 'Продление чата', () => {
      setChatTimer(prev => prev + 900);
      setChatMessages(prev => [...prev, {
        id: `extend-${Date.now()}`, sender: 'system',
        text: '⏰ Чат продлен на 15 минут',
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }]);
    });
  };
  ...
  const handleUnlockMedia = () => {
    ...
    handleSpendStars(10, 'Разблокировка медиа', () => {
      setMediaUnblocked(true); setMediaTimer(300);
    });
  };
  ...
  const handleSendPhotoMock = () => {
    ...
    handleSendMessage(undefined, {
      type: 'media',
      content: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=400&q=80'
    });
  };
  ...
  const handleActivateRoomPlus = () => {
    ...
    handleSpendStars(199, 'Подписка Room+', () => setIsRoomPlus(true));
  };
  ...
  return (
    <div className="min-h-screen bg-[#050506] text-white font-sans flex flex-col">
      ...
      {/* CONTENT */}
      <div className="flex-1 overflow-y-auto bg-radial-gradient-lobby">
        ...
        {activeTab === 'chat' && (
          <div className="flex flex-col h-full">
            ...
            <form onSubmit={handleSendMessage} className="p-3 border-t border-white/[0.05] bg-[#101012] flex gap-2">
              <button type="button" onClick={handleSendPhotoMock} className="p-2 bg-white/5 rounded-xl">
              ...
              <input
                ...
                className="flex-1 bg-[#1C1C1F] rounded-xl px-4 text-sm text-white focus:outline-none focus:ring-1 focus:ring-purple-500"
              />
        ...
        {activeTab === 'profile' && (
          <div className="p-6 space-y-6">
            ...
            <div className="bg-[#1C1C1F] p-4 rounded-xl border border-white/5">
              <h3 className="font-bold mb-2">Подписка Room+</h3>
              <button 
            ...
            <div className="bg-[#1C1C1F] p-4 rounded-xl border border-white/5">
              ...
              <div className="flex gap-2">
                <input 
                  readOnly 
                  value="t.me/RoomOfSecretsBot?start=ref_123" 

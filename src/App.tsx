          {/* ----------------------------------------------------
              B. CHAT TAB (FIXED LAYOUT)
              ---------------------------------------------------- */}
          {activeTab === 'chat' && (
             <div className="flex flex-col h-full bg-radial-gradient-lobby">
              
              {/* 1. Chat Session Header (Fixed Top) */}
               <div className="bg-[#101012] px-3.5 py-2.5 border-b border-white/[0.05] flex items-center justify-between shrink-0 z-10">
              ...
              {/* 2. MESSAGES VIEW (Scrollable Area) */}
               <div className="flex-1 overflow-y-auto p-3.5 space-y-3 flex flex-col bg-[#08080a] custom-chat-scroll relative">
                        ...
                        {msg.type === 'media' ? (
                           <div className="space-y-1.5">
                             <img 
                              src={msg.text} 
                              alt="Media upload" 
                              className="rounded-lg max-h-40 object-cover w-full opacity-90"
                            />
              ...
              {/* 3. CHAT PREMIUM CONTROLS TRAY (Fixed Bottom Area) */}
               <div className="bg-[#101012]/90 border-t border-white/[0.04] p-2 flex items-center justify-between space-x-1.5 shrink-0 backdrop-blur-sm">
                
                {/* Media attachments */}
                 <button
                  onClick={handleUnlockMedia}
                  className={`px-2 py-1.5 rounded-lg text-[10px] flex items-center space-x-1 border transition-all ${
                 ...
                 <div className="flex items-center space-x-1">
                  {/* Extend chat button */}
                   <button
                    ...
                    className="px-2 py-1.5 rounded-lg bg-[#1C1C1F] border border-white/[0.05] text-amber-300 hover:border-amber-300/30 text-[10px] flex items-center space-x-1 transition-all"
              ...
              {/* 4. INPUT BAR (Fixed Bottom) */}
               <form onSubmit={handleSendMessage} className="bg-[#0A0A0B] p-2.5 border-t border-white/[0.05] flex items-center space-x-2 shrink-0">
                 ...
                 <button
                  ...
                  title={mediaUnblocked || isRoomPlus ? "Отправить фото" : "Купите разблокировку"}
                 >
                 ...
                 <button
                  ...
                  title={mediaUnblocked || isRoomPlus ? "Записать голосовое" : "Купите разблокировку"}
                 >
